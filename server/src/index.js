...
// --- 1. CONFIG & MIDDLEWARE ---
app.set('trust proxy', 1); // За Render.com
...
app.post('/api/newsletter/subscribe', async (req, res) => {
    ...
    try {
        await db.query('INSERT INTO newsletter_subscribers (email) VALUES ($1) ON CONFLICT (email) DO NOTHING', [email]);
...
app.get('/api/newsletter/subscribers', adminMiddleware, async (req, res) => {
    try {
        const { rows } = await db.query('SELECT email, created_at FROM newsletter_subscribers ORDER BY created_at DESC');
        res.json(rows);
...
app.post('/api/newsletter/send', adminMiddleware, async (req, res) => {
    ...
    try {
        ...
        if (rows.length === 0) return res.status(400).json({ error: "No subscribers found" });
...
app.post('/api/magazines', adminMiddleware, async (req, res) => {
    ...
    try {
        const { rows } = await db.query(
            `INSERT INTO magazine_issues (issue_number, month, year, is_locked, cover_url, pages) 
...
app.post("/api/auth/register", async (req, res) => {
  ...
  try {
    ...
    await transporter.sendMail({ from: '"MIREN" <mirenmagazine@gmail.com>', to: email, subject: 'Confirm Account', html: `<a href="${confirmationUrl}">Click to Confirm Email</a>` });
...
app.post('/api/auth/login', async (req, res) => {
  ...
  try {
    ...
    if (!user.is_confirmed) return res.status(403).json({ error: 'Confirm email first' });
...
app.post('/api/auth/confirm', async (req, res) => {
  ...
  try {
    ...
    await db.query('UPDATE users SET is_confirmed = true, confirmation_token = NULL WHERE email = $1', [rows[0].email]);
...
app.get('/api/user/me', authMiddleware, async (req, res) => {
  try {
    ...
    res.json({ email: rows[0].email, displayName: rows[0].display_name, twoFaEnabled: rows[0].two_fa_enabled });
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
app.post('/api/auth/reset-password-request', async (req, res) => {
    ...
    try {
        ...
        await transporter.sendMail({ to: email, subject: 'Reset Password', html: `<a href="${url}">Reset Here</a>` });
        res.json({ ok: true });
...
app.post('/api/auth/send-2fa', async (req, res) => {
    ...
    try {
        ...
        await transporter.sendMail({ to: email, subject: '2FA Code', html: `Code: ${code}` });
        res.json({ ok: true });
    } catch (e) { res.status(500).json({ error: "Error" }); }
});
...
app.post('/api/auth/verify-2fa', async (req, res) => {
    ...
    try {
        const { rows } = await db.query('SELECT * FROM users WHERE email = $1', [email]);
        const user = rows[0];
        if (user.two_fa_code !== code || new Date() > new Date(user.two_fa_expiry)) return res.status(401).json({ error: "Invalid/Expired" });
        await db.query('UPDATE users SET two_fa_enabled = true, two_fa_code = NULL WHERE email = $1', [email]);
        const token = signToken({ email: user.email });
...
app.post('/api/create-checkout-session', authMiddleware, async (req, res) => {
  ...
  let priceData;
  if (plan === 'monthly') priceData = { product_data: { name: 'Monthly' }, unit_amount: 499, currency: 'eur', recurring: { interval: 'month' } };
  else priceData = { product_data: { name: 'Yearly' }, unit_amount: 4999, currency: 'eur', recurring: { interval: 'year' } };
  
  try {
    const session = await stripe.checkout.sessions.create({
      payment_method_types: ['card'], mode: 'subscription', line_items: [{ price_data: priceData, quantity: 1 }],
      customer_email: userEmail, success_url: `${APP_URL}/profile?payment_success=true`, cancel_url: `${APP_URL}/subscriptions`,
    });
...
app.post("/api/contact", async (req, res) => {
  ...
  await transporter.sendMail({ from: email, to: process.env.EMAIL_USER, subject: `Contact`, text: message });
  res.json({ ok: true });
});
